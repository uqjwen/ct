...
基于提交 `a81c012` 复核 AG→DC 流水、borrow、D-cache tag/data 选择、restart、RVV unit-stride 元数据和调试地址触发接口。当前未提供 D-cache arbiter、MMU 和门控时钟单元实现，跨模块项按合同依赖分类。

| ID | 优先级 | 置信度 | 状态 | 摘要 |
|---|---:|---|---|---|
| DC-RR-01 | P2 | 已确认 | 开放 | `ld_dc_dtu_addr_vld` 置 1 后没有正常清零路径 |
| DC-RR-02 | P1 | 合同依赖 | 条件关闭 | arbiter 声明 valid/gate 等价；consumer 仍需边界断言 |
...
若 `borrow_vld=1` 而 gate=0，下一拍会带有效 borrow 和旧 payload 进入 DA。Interaction 1.3 已明确二者在 `xx_lsu_dcache_arb` 中等价，因此本项按合同条件关闭；该 producer 不在仓库中，仍必须在 arbiter/DC 边界断言双向等价 `borrow_vld == borrow_vld_gate`，而不只检查单向覆盖。
...
borrow 地址不是独立 bug：AG 在 borrow address valid 时已经把 arbiter 地址复用到 `lag_ldc_ex1_addr0`，DC 再统一采样。DC 的 ahead-WB 预测也不会绕过 restart，因为 DA 只在 `ldc_lda_ex2_inst_vld` 下锁存该 valid。Interaction 1.3 没有改变已确认的 DC-RR-01；当前首要修复仍是调试 valid 粘住，然后在完整 dcache arbiter/一致性环境固化 borrow 等价和 tag one-hot 合同。

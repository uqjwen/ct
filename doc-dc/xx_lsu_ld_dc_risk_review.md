...
| DC-RR-03 | P1 影响 | 源码+合同 | RTL 断言已实现，待动态运行 | live cacheable load 的多 way tag hit 由 `$onehot0` 就地监测 |
...
Interaction 1.3 已确认 LFB/一致性逻辑保证同 set 同 tag 最多一路有效，因此本项按合同关闭；缓存一致性数据比对能在系统级发现副本错误，但 DC 的 OR 编码仍可能把根因推迟成数据错。Interaction 1.6 已在 DC 命中点加入 `$onehot0(ldc_hit_way)` assertion；仍需用重复 tag 负向注入验证即时可见性。
...
## 5. Interaction 1.6：DC-RR-03 断言已实现

本轮已在 `ldc_hit_way` 形成处加入 synthesis-excluded assertion：live EX2、cacheable page 且 D-cache enabled 时必须满足 `$onehot0(ldc_hit_way)`。使用 onehot0 是为了允许合法 miss；2 到 4 路同时命中会作为 duplicate tag/coherence failure 立即报告。DC-RR-03 从“合同待固化”更新为“RTL 监测已实现，仍待完整一致性环境运行”。

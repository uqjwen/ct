...
以 `{lane, IID, observer_generation}` 为主键，记录 `{source(cache/TCM/SQ/WMB/RB), four_128b_blocks, byte_mask, ECC_phase, RB_entry, debug_cancel}`。`observer_generation` 由验证环境在 entry/stage 复用时递增，不要求加入 RTL。completion、data writeback、RB create/merge、LQ pop、LRQ bitmap 和辅助训练事件分别计数，但必须关联同一 live transaction；flush/restart 后旧 owner 不得再产生功能 side effect。
...
同一测试复制到 LSDA0/LSDA1，并增加 scalar-vs-LSDA 等价比较；Interaction 1.5 源码已满足 block3 identity，3 lane 动态回归通过后关闭 DA-RR-01。

## 3. DA-RR-02 至 DA-RR-05 必跑矩阵

| 风险 ID | 激励 | 必查结果 |
|---|---|---|
| DA-RR-02 | SQ full/partial/bypass forward × ECC first/stall-already/fatal 全交叉 | 规格若声明互斥则零次同时有效；否则明确 discard/reissue，不能跨 owner 合并 cache/SQ 数据 |
| DA-RR-03 | RB empty/one-left/full、create/merge、ECC mask、index hit/discard 与 flush 同拍 | 每个 live miss 恰好 `accept XOR restart`；RB entry 的 IID/payload 与 DA 保存事务相同，observer generation 不串代 |
...
以下 assertion 需要放在能观察内部信号的 bind/testbench 层；`transaction_owner_live` 与 `rb_accept_same_owner` 由验证环境 scoreboard 提供。
...
assert property (@(posedge ctrl_ld_clk) disable iff (!cpurst_b)
  ld_da_rb_create_vld_unmask && !lda_rb_ex3_ecc_mask
  |-> (rb_accept_same_owner ^ ld_da_data_rb_restart));

a_da_rr03_ecc_hold_owner:
assert property (@(posedge ctrl_ld_clk) disable iff (!cpurst_b)
  ld_da_rb_create_vld_unmask && lda_rb_ex3_ecc_mask
  |=> lda_ex3_inst_vld && $stable({lda_ex3_iid, expected_four_block_payload}));

a_da_rr03_side_effect_owner_live:
assert property (@(posedge ctrl_ld_clk) disable iff (!cpurst_b)
  (lda_lwb_ex3_cmplt_req || lda_lwb_ex3_data_req ||
   lda_rb_ex3_create_vld || lda_rb_ex3_merge_vld)
  |-> transaction_owner_live);
...
- DA-RR-01（历史 P1）：3 lanes × 16 ECC banks × `{tag,data,TCM}` × `{correctable,fatal}` × 4 个互异数据块。
...
只有在标量 ECC replay 动态证明不再复制 block2、SQ/ECC 合同有唯一解释、RB accept/restart 零次丢失或双收、debug 副作用符合书面合同、所有 assertion 为零失败后，DA-RR-01 至 DA-RR-04 才可完成动态签核。

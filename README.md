...
# interaction 2.0
***
1. 所列的功能点或者测试计划过于简单，工程师获得该功能点或者测试计划无法直接编写测试用例，将功能点或者测试计划更详细一点，以“当***信号满足\*\*\*条件时，则触发\*\*\*情况”。
***

# interaction 2.1
***
1. 功能点和测试计划还是不够细致，比如AG-FP-05中，就缺少了以下场景：当lag_ex1_stall_ori=1, ld_ag_stall_mask=1， mmu_lsu_pa_vld=0, lsu_mmu_abort=1，也就是当在ex1的停顿请求被更老的rf栈请求覆盖时，虽然发生了tlb-miss，但是mmu请求被abort了，仍然需要立即重发。请仔细检查，举一反三，重新生成功能点和测试计划。
...
3. 按照excel格式生成waive列表时，不需要所有栏目都填充，只需要填充对象位置（所在模块-行数）、所属子模块（LSU或者CP0）、排除原因，上述3栏即可，将LSU的排除列表也按照上述3栏生成。有些章节由于同一原因排除的，比如1.1.4和1.2.4，每个信号的toggle排除单独列一行，不要堆在同一行。
###

# interaction 2.3
###
1. cp0目录添加了cp0相关的rtl代码，请据此生成详细设计文档，注：开发系统级中断异常的验证人员需要参考该设计文档。
###
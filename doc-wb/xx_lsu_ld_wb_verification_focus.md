...
| WB-V04 | 单个 RB data-check 后停止所有 DA/RB/VMB/WMB data request，等待 DTU 两级返回 | `ld_dtu2_vld` 打开 source clock；registered update 严格一拍并由自身打开清零边沿；不得粘连 |
...
- 空闲无后续 data traffic 下打一拍 `ld_dtu2_vld`，必须看到 update 下一拍自清、effect 再下一拍清零；已完成静态三拍模型，仍需门控时钟仿真。
- 分别持续占用 0/1/2/3 条 dedicated lane，并让 WMB/VMB/RB 请求持续有效；safety assertion 必须 0 failure。
- 若声称 WB-RR-04 waive，必须提供 `WB_GRANT_BOUND` 或“高优先级流量在 N 拍内最终让路”的可验证系统合同；不能只引用 C910 单通道签核。

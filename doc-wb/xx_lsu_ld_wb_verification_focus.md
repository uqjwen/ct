...
| WB-V04 | 单个 RB data-check 后停止所有 DA/RB/VMB/WMB data request，等待 DTU 两级返回 | raw update event 必须自行打开 source clock、形成单拍 update，并打开 completion clock更新 halt-info |
| WB-V05 | halt bit0/bit1 单独和组合，随后停止所有 completion/update 流量 | bit1 clear event 必须自行开钟；`11` 按规格只维持一拍，旧 bit0 不得重复 completion |
...
| WB-V08 | 持续高优先级流量、ECC dp-only、issue-queue 填满与真正 `xx_lsu_wb_arbiter` 三 lane 迁移交叉，夹带 RB/WMB/VMB 请求 | loser 保持 payload并在规定上界内获 grant，或正式触发 starvation 监控；BIU bus arb不能替代此用例 |

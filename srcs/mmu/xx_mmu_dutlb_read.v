//-----------------------------------------------------------------------------
// File          : xx_mmu_dutlb_read.v
// Created       : 2024/10/01 (by Li Shuo)
// Last modified : 2024/11/18 (by Huang chenglong)
...
// $Id: xx_mmu_dutlb_read.vp,v 1.63 2022/01/06 08:14:36 jizk Exp $
// *****************************************************************************

// &ModuleBeg; @23
module xx_mmu_dutlb_read #(
  ...
  sysmap_mmu_flg_x,
  utlb_req_vpn_x,
  refill_vpn,        // tmq fixed@hcl   20251021
  refill_pgs      // tmq fixed@hcl   20251021
);
...
input           entry0_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry0_ppn;               
input           entry0_vld;               
input   [13:0]  entry10_flg;              
input           entry10_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry10_ppn;              
input           entry10_vld;              
input   [13:0]  entry11_flg;              
input           entry11_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry11_ppn;              
input           entry11_vld;              
input   [13:0]  entry12_flg;              
input           entry12_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry12_ppn;              
input           entry12_vld;              
input   [13:0]  entry13_flg;              
input           entry13_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry13_ppn;              
input           entry13_vld;              
input   [13:0]  entry14_flg;              
input           entry14_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry14_ppn;              
input           entry14_vld;              
input   [13:0]  entry15_flg;              
input           entry15_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry15_ppn;              
input           entry15_vld;              
input   [13:0]  entry16_flg;              
input           entry16_hit_x;            
input   [`WK_PPN_WIDTH-1:0]  entry16_ppn;              
input           entry16_vld;              
input   [13:0]  entry1_flg;               
input           entry1_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry1_ppn;               
input           entry1_vld;               
input   [13:0]  entry2_flg;               
input           entry2_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry2_ppn;               
input           entry2_vld;               
input   [13:0]  entry3_flg;               
input           entry3_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry3_ppn;               
input           entry3_vld;               
input   [13:0]  entry4_flg;               
input           entry4_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry4_ppn;               
input           entry4_vld;               
input   [13:0]  entry5_flg;               
input           entry5_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry5_ppn;               
input           entry5_vld;               
input   [13:0]  entry6_flg;               
input           entry6_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry6_ppn;               
input           entry6_vld;               
input   [13:0]  entry7_flg;               
input           entry7_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry7_ppn;               
input           entry7_vld;               
input   [13:0]  entry8_flg;               
input           entry8_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry8_ppn;               
input           entry8_vld;               
input   [13:0]  entry9_flg;               
input           entry9_hit_x;             
input   [`WK_PPN_WIDTH-1:0]  entry9_ppn;               
input           entry9_vld;               
input           forever_cpuclk;           
input           lsu_mmu_abort_x;          
input   [IID_WIDTH-1 :0]  lsu_mmu_id_x;             
input   [`WK_PPN_WIDTH-1:0]  lsu_mmu_stamo_pa_x;       
input           lsu_mmu_stamo_vld_x;      
input           lsu_mmu_va_vld_x;         
input   [63:0]  lsu_mmu_va_x;             
input   [`WK_PPN_WIDTH-1:0]  lsu_mmu_vabuf_x;          
input           pad_yy_icg_scan_en;       
input   [3 :0]  pmp_mmu_flg_x;            
input   [IID_WIDTH-1 :0]  refill_id_flop;           
input   [4 :0]  sysmap_mmu_flg_x;   
input   [`WK_VPN_WIDTH-1:0]  refill_vpn; 
input   [2 :0]  refill_pgs;    
output          dutlb_acc_flt_x;          
output          dutlb_inst_vpn_match_x;    
...
output  [`WK_PPN_WIDTH-1:0]  mmu_lsu_pa_x;             
output          mmu_lsu_page_fault_x;     
output          mmu_lsu_sec_x;            
output          mmu_lsu_sh_x;             
output          mmu_lsu_so_x;             
output          mmu_lsu_stall_x;          
output  [`WK_PPN_WIDTH-1:0]  mmu_pmp_pa_x;             
output  [`WK_PPN_WIDTH-1:0]  mmu_sysmap_pa_x;          
output  [`WK_VPN_WIDTH-1:0]  utlb_req_vpn_x;           

// &Regs; @25
reg     [13:0]  dutlb_entry_flg;          
reg     [`WK_PPN_WIDTH-1:0]  dutlb_entry_pa;           
reg     [`WK_PPN_WIDTH-1:0]  dutlb_pa_buf;             
...
wire    [`WK_PPN_WIDTH-1:0]  dutlb_fin_pa;             
wire    [2 :0]  dutlb_fin_pgs;            
wire            dutlb_hit_vld;            
wire            dutlb_inst_vpn_hit;     
wire            dutlb_inst_vpn_huge_hit; 
wire            dutlb_inst_vpn_normal_hit;   
wire            dutlb_inst_vpn_match_x;    
wire            dutlb_inst_id_older_x;    
wire            dutlb_miss_vld_short_x;   
wire            dutlb_miss_vld_x;         
wire    [13:0]  dutlb_off_flg;            
wire            dutlb_off_hit;            
wire    [`WK_PPN_WIDTH-1:0]  dutlb_off_pa;             
...
wire    [`WK_PPN_WIDTH-1:0]  dutlb_pre_pa;             
...
wire    [IID_WIDTH-1 :0]  lsu_mmu_id_x;             
wire    [`WK_PPN_WIDTH-1:0]  lsu_mmu_stamo_pa_x;       
...
wire    [`WK_PPN_WIDTH-1:0]  mmu_lsu_pa_x;             
...
wire    [`WK_PPN_WIDTH-1:0]  mmu_pmp_pa_x;             
wire    [`WK_PPN_WIDTH-1:0]  mmu_sysmap_pa_x;          
wire    [17:0]  pa_offset;                
wire            pabuf_clk;                
wire            pabuf_clk_en;             
...
wire    [`WK_VPN_WIDTH-1:0]  utlb_req_vpn_x;           
wire    [16:0]  vpn_hit;                  
wire    [16:0]  vpn_vld;                  
...
assign dutlb_va_illegal =(mmu_sv48_en ? (lsu_mmu_va_x[VPN_WIDTH+11] && !(&lsu_mmu_va_x[63:VPN_WIDTH+12])
                      ||  !lsu_mmu_va_x[VPN_WIDTH+11] &&  (|lsu_mmu_va_x[63:VPN_WIDTH+12]))
                      : (lsu_mmu_va_x[VPN_WIDTH+2] && !(&lsu_mmu_va_x[63:VPN_WIDTH+2])
                      ||  !lsu_mmu_va_x[VPN_WIDTH+2] &&  (|lsu_mmu_va_x[63:VPN_WIDTH+2]))
)
...
begin
  ...
  else if(lsu_mmu_va_vld_x && |dutlb_entry_hit[15:0])
...
assign dutlb_miss_vld_x = lsu_mmu_va_vld_x
                     ...
                     && !lsu_mmu_abort_x
                     && !dutlb_off_hit
                     && !(dutlb_inst_vpn_hit // wjh@tmq
                          && (dutlb_ref_pgflt || dutlb_ref_acflt)); // wjh@tmq
assign dutlb_miss_vld_short_x = lsu_mmu_va_vld_x
                     && !dutlb_entry_hit_vld
                     && !dutlb_va_illegal
                     && !dutlb_off_hit;

// &Force("output", "dutlb_inst_vpn_match_x"); @137
// assign dutlb_inst_vpn_huge_hit     = (refill_vpn[VPN_WIDTH-1:VPN_WIDTH-VPN_PERLEL] == lsu_mmu_va_x[VA_WIDTH-1:VA_WIDTH-VPN_PERLEL])
//                                 && lsu_mmu_va_vld_x;
assign dutlb_inst_vpn_huge_hit     = (refill_vpn[VPN_WIDTH-1:18] == lsu_mmu_va_x[VA_WIDTH-1:30])
                                && lsu_mmu_va_vld_x;
assign dutlb_inst_vpn_normal_hit     = (refill_vpn[VPN_WIDTH-1:0] == lsu_mmu_va_x[VA_WIDTH-1:VA_WIDTH-VPN_WIDTH])
                                && lsu_mmu_va_vld_x;
// assign dutlb_inst_vpn_hit = refill_pgs[2] & dutlb_inst_vpn_huge_hit | (|refill_pgs[1:0]) & dutlb_inst_vpn_normal_hit;
assign dutlb_inst_vpn_hit = dutlb_inst_vpn_normal_hit;
assign dutlb_inst_vpn_match_x = dutlb_inst_vpn_hit && !lsu_mmu_abort_x;

// &Instance("wk_rtu_compare_iid","x_mmu_dutlb_read_compare_req_iid"); @142
wk_rtu_compare_iid  #(.IID_WIDTH(IID_WIDTH)) x_mmu_dutlb_read_compare_req_iid (
  .x_iid0              (lsu_mmu_id_x[IID_WIDTH-1:0]  ),
...
assign dutlb_expt_match = dutlb_expt_for_taken && dutlb_inst_vpn_hit;
...
//----------------------------------------------------------
// &CombBeg; @185
always @( entry4_ppn[`WK_PPN_WIDTH-1:0]
       or entry1_ppn[`WK_PPN_WIDTH-1:0]
       or entry8_ppn[`WK_PPN_WIDTH-1:0]
       or dutlb_entry_hit[15:0]
       or entry13_ppn[`WK_PPN_WIDTH-1:0]
       or entry7_ppn[`WK_PPN_WIDTH-1:0]
       or entry10_ppn[`WK_PPN_WIDTH-1:0]
       or entry11_ppn[`WK_PPN_WIDTH-1:0]
       or entry0_ppn[`WK_PPN_WIDTH-1:0]
       or entry2_ppn[`WK_PPN_WIDTH-1:0]
       or entry3_ppn[`WK_PPN_WIDTH-1:0]
       or entry6_ppn[`WK_PPN_WIDTH-1:0]
       or entry5_ppn[`WK_PPN_WIDTH-1:0]
       or entry14_ppn[`WK_PPN_WIDTH-1:0]
       or entry15_ppn[`WK_PPN_WIDTH-1:0]
       or entry9_ppn[`WK_PPN_WIDTH-1:0]
       or entry12_ppn[`WK_PPN_WIDTH-1:0])
begin
...
assign dutlb_off_pa[PPN_WIDTH-1:0] = lsu_mmu_va_x[PPN_WIDTH+11:12];
...
assign jtlb_acc_fault = dutlb_ref_acflt && dutlb_refill_on_x && dutlb_inst_vpn_match_x; 
...
//           .module_en  (cp0_mmu_icg_en), @295
//           .local_en   (pabuf_clk_en  ), @296
//           .clk_out    (pabuf_clk     ) @297
//          ); @298
...
assign mmu_sysmap_pa_x[PPN_WIDTH-1:0] = lsu_mmu_va_x[PPN_WIDTH+11:12];

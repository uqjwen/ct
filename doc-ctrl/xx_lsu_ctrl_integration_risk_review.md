...
基于提交 `a81c012` 完成初审，并在 Interaction 1.5 提交 `443384a` 上复核全局/pipe 门控时钟、LRQ 迁移后的 restart/wakeup bitmap、flush/参数合同、MMU/LFB/SQ/WMB/LSDA producer 和新 WB arbiter。仓库仍缺部分宏头文件、helper、完整 filelist 和 testbench，因此只能完成 source-level 结构审查，不能声称已完整 elaboration。
...
### CTRL-RR-03：无 epoch wakeup bitmap 的 owner-lifetime 不变量
...
既有可见模块的平衡括号和 named-port 集合检查未发现重复端口；Interaction 1.5 已补充 top 实例化的 `xx_lsu_wb_arbiter`（`srcs/xx_lsu_top.sv:10594-10627`、`srcs/xx_lsu_wb_arbiter.sv:29-149`），其 instance port 集合完整。clock cell、vector/helper、宏定义与可执行 filelist 仍未完整提供，结构文本检查不能替代编译/elaboration。

## 4. 处理顺序

现配置首先应保留 `pfu_part_empty=0` 的显式配置断言；若要恢复时钟门控，补齐 ICC/LSDA 三个 enable。随后固化 `LRQENTRY==LSIQENTRY` 和所有异步 wakeup 的 owner-lifetime 合同，并在完整 SoC 源码环境运行 lint/elaboration。

...
| RB-RR-02 | — | 源码+上游+合同 | debug-recovery 范围豁免 | async flush 用于 hung outstanding 后进入 debug，不要求被丢弃请求返回；可恢复 resume 不在该豁免内 |
| RB-RR-03 | P2 | 合同依赖 | 条件关闭 | US response 用本地 1-bit beat counter，不检查 `r_last`/response 次数 |
| RB-RR-04 | P2 影响 | 源码+上游+合同 | C910 签核条件豁免 | 未按 state 限定的 B hit 是继承行为；waive 需接受 C910 签核并确认本地固定 ID 唯一 owner 合同未改 |
...
此外，本地普通 non-cacheable 分支已把上游固定 `BIU_R_NC_ID=24` 改为 `lfb_rb_create_id`（本地 `srcs/xx_lsu_rb.sv:1829-1836` 对比[上游 `ct_lsu_rb.v:2488-2507`](https://github.com/T-head-Semi/openc910/blob/b91c90914c19f114d35c8f6b73408eb241ed847c/C910_RTL_FACTORY/gen_rtl/lsu/rtl/ct_lsu_rb.v#L2488-L2507)），因此不能直接继承上游“可恢复后继续执行”的 ID 复用证明。Interaction 1.6 已把目标范围限定为 hung outstanding 后进入 debug 查看状态、被丢弃请求不要求返回；在这个不可恢复事务边界内本项豁免。若还要求不复位 interconnect 就 resume，必须重新启用 drain/tombstone/ID-reuse 检查。
...
generate entry named-port 集合一致，WB completion/data 均先形成 one-hot pointer再选择 payload；文本复查未见重复端口。本轮未发现 Interaction 1.3 所担心的 create payload 串线。建议保留 create-winner/payload-source identity assertion，固化 SO ID FIFO、US 两拍和 paired WMB/RB 固定 ID ownership；只有要求 debug resume 时才追加 async-flush response drain/tombstone，再处理 ECC 配置。

## 5. Interaction 1.6：RB-RR-02/04 豁免结论

### RB-RR-02

本次新增合同明确 async flush 是“outstanding 长期不返回时进入 debug 查看核状态”的恢复手段，未提交请求不要求收到响应。按此不可恢复事务边界，RB 在 WAIT_RESP/REQ_WB 直接回 IDLE 可豁免，不再要求等待 response drain。

该豁免不包含“无 interconnect reset 可继续正常执行”。若 debug resume 需要复用旧 ID，仍必须证明迟到 response 被 tombstone/丢弃或总线侧已复位；否则原 ID-reuse 风险重新开放。

### RB-RR-04

本地 `rb_entry_biu_b_resp_set = rb_entry_b_id_hit` 与官方 C910 相同，WMB 的 B-ID valid 也只在正式 write request/mem-set grant 后置位。若项目接受 C910 的既有签核，并确认本工程 RB/WMB/BIU 固定 ID 编码和全局唯一 outstanding 合同未改，可条件 waive。源码相同只能证明继承关系，不能替代 C910 的验证/量产签核证据。

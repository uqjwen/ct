...
| V0-NS-REPLAY | 同一组 no-spec 请求 create 后随机等待 0/1/N 拍再 replay | 改义后的 `no_spec_exist` 准确表示内部 wait 历史；replay 的旧 DA/WB no-spec 反馈即使含 X，也不得被 SFP/IDU/RTU 消费、不得更新标签/预测状态或触发 flush | `srcs/xx_lsu_lrq.sv:1792-1794`、`srcs/xx_lsu_ld_da.sv:5318-5372`、`srcs/xx_lsu_ld_wb.sv:1593-1596`；条件关闭 RR-02 |
| V0-REPLAY-X | scalar、boundary LDR、vector indexed、vector US 分别 replay，开启 X-prop | saved VA/boundary/byte/reg-byte mask 与 create 时一致；相同 saved `split_num/vmew` 必须重建相同且二态的 `element_cnt/rot_sel`，rotation、FOF VL/vstart 与参考模型一致 | `srcs/xx_lsu_lrq.sv:1805,1822`、`srcs/xx_lsu_ld_ag.sv:1692-1756,2250-2260`；条件关闭 RR-02 |
...
7. `vector/no-spec replay -> saved VA/byte mask -> DC/DA/WB`：旧 no-spec 反馈必须被新 SFP/IDU 所有权屏蔽；`element_cnt/rot_sel` 必须由原事务的 saved `split_num/vmew` 等价重建且不含 X；FOF fault 的 VL/vstart 必须正确。
...
- replay 时即使 LRQ 的 `VL/vmask/source mask` 为 X，valid `element_cnt/rot_sel` 也必须仅由 saved `split_num/vmew` 等价重建且不得为 X；若 helper 实际依赖其他未保存字段，interaction 1.1.3 合同失效，必须保存/恢复最终派生值；
...
- 复位后首条请求；
- idle MMU 输出为 X；
- replay 的旧 `no_spec`、VL/vmask/source mask 为 X，检查旧 no-spec 反馈不被消费且 vector helper 输出保持二态；
...
// 有效 DC 的功能 payload 不能含 X。interaction 1.1.3 已把 replay no_spec
// 定义为遗留死字段，因此不在此通用 payload 断言中检查。
a_no_x_on_valid_dc:
assert property (@(posedge forever_cpuclk) disable iff (!cpurst_b)
  lag_ldc_ex1_inst_vld
...
// RR-02：活动 vector helper 必须仅用已保存的 split_num/vmew 等价重建。
a_replay_vector_metadata_known:
assert property (@(posedge forever_cpuclk) disable iff (!cpurst_b)
  (lag_ldc_ex1_inst_vld && lag_lrq_replay_vld)
  |-> !$isunknown({lag_ldc_ex1_element_cnt,
                   lag_ldc_ex1_rot_sel}));

// RR-02：txn_is_replay 由 scoreboard 跟踪到 completion；正式工程应把
// no_spec_feedback_consumed 替换成 SFP/IDU/RTU 的真实更新/flush 条件。
a_replay_legacy_no_spec_not_consumed:
assert property (@(posedge forever_cpuclk) disable iff (!cpurst_b)
  txn_is_replay |-> !no_spec_feedback_consumed);
...
4. RR-02 的 SFP/IDU 所有权与 vector helper 等价断言已通过；RR-03 的 `halt_info` 已补齐；saved VA/byte masks 与 RR-12 的 27 组定向测试全部通过，且 RR-12 的 any-X/functional-ambiguity 政策已有书面结论。
...
1. RR-02：请给出 SFP/IDU 的正式 assertion，证明 replay 后旧 DA/WB no-spec 反馈不会更新新标签、预测状态或触发 flush；否则请在 LSU 内屏蔽或确定化。
2. RR-02：请补 helper 源码或等价断言，证明 vector replay 的 `element_cnt/rot_sel` 仅由 saved `split_num/vmew` 唯一重建；若不能，请把最终派生值纳入 LRQ payload。
